    ...
    <div class="">
        ...
        <div class="modal-body mb-5">
            <form action="{{ route('sector.store') }}" method="POST">
                @csrf
                <input class="w-full border p-3" name="sector" type="text" placeholder="বিভাগ লিখুন">
                ...
                <br>
                <input class="w-full border p-3" name="prefix" type="text" placeholder="পিফিক্স লিখুন">
                <small class="text-danger block mb-3">{{ $errors->first('prefix') }}</small>
                <button class="text-md bg-primary text-light py-1 px-4 rounded">নতুন বিভাগ তৈরী করুন</button>
            </form>
        </div>
    </div>
</div>
